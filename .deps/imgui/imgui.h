...
enum ImGuiCol_
{
    ...
    ImGuiCol_ChildShadow,           // Child window shadows
    ImGuiCol_COUNT,
...
struct ImGuiStyle
{
    ...
    float       ChildShadowSize;            // Size (in pixels) of child window shadows. Set this to zero to disable shadows.
    float       ChildShadowOffsetDist;      // Offset distance (in pixels) of child window shadows from casting child.
    float       ChildShadowOffsetAngle;     // Offset angle of child window shadows from casting child (0.0f = left, 0.5f*PI = bottom, 1.0f*PI = right, 1.5f*PI = top).
...
struct ImDrawList
{
    ...
    IMGUI_API void  AddShadowRectRounded(const ImVec2& obj_min, const ImVec2& obj_max, ImU32 shadow_col, float shadow_thickness, const ImVec2& shadow_offset, float obj_rounding, ImDrawFlags flags = 0);
